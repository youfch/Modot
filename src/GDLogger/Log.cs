...
namespace Godot
{
    ...
    public static class Log
    {
        static Log()
        {
            // File I/O is deliberately not performed here: the log file is opened lazily by OpenFile(),
            // so that merely loading this type stays safe in a process without the Godot runtime.
        ...
        private static string filePath = Log.defaultFilePath;
        ...
        public static string FilePath
        {
            get
            {
                return Log.file?.GetPathAbsolute() ?? Log.filePath;
            }
            set
            {
                // If a previous log file is already open, save its contents and close it
                if (Log.file is not null)
                {
                    Log.Flush(true);
                    Log.file.Close();
                    Log.file = null;
        ...
        public static void Write(Entry entry)
        {
            ...
            Log.OpenFile()?.StoreLine(entry.ToString());
        ...
        /// <summary>
        /// Opens the log file if it is not open yet.
        /// </summary>
        /// <remarks>
        /// Kept out of the static constructor on purpose: upstream opened the file while initialising the type,
        /// which performs I/O — and throws — outside the engine. The public surface and the file format are unchanged.
        /// </remarks>
        /// <returns>The open <see cref="FileAccess"/>, or <see langword="null"/> if it could not be opened.</returns>
        private static FileAccess? OpenFile()
        {
            Log.file ??= FileAccess.Open(Log.filePath, FileAccess.ModeFlags.Write);
            return Log.file;
        }
